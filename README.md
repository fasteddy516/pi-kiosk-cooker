...
| `squeekboard` / `maliit-keyboard` _(when available)_ | Wayland on-screen keyboard. `squeekboard` is preferred for wlroots/labwc reliability, with `maliit-keyboard` as fallback. For Maliit, the kiosk session starts `maliit-server` on Wayland so text fields can trigger pop-up keyboard behavior. |
...
**`labwc/autostart`** is a shell script that labwc executes at session start. When Raspberry Pi Connect is enabled, it imports the Wayland session environment into systemd and D-Bus so that `rpi-connect-wayvnc.service` can reach the compositor for screen sharing. If a supported touch keyboard package is available (`squeekboard` preferred, otherwise `maliit-keyboard`), the script also starts it here so the on-screen keyboard can appear for text input. On Maliit installs this starts `maliit-server` with `QT_QPA_PLATFORM=wayland`.
